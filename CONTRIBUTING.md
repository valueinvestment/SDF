...
SDF는 안정적인 메인 배포 버전 유지를 위해 아래와 같은 Git 워크플로우를 가집니다.

* `main`: 상용 프로덕션 환경에 배포되는 가장 안정적인 브랜치입니다.
* `develop`: 개발의 중심이 되는 통합 브랜치입니다. 모든 기여 작업은 이 브랜치를 기반으로 진행됩니다.
* `feature/기능명`: 새로운 기능, 플러그인, 아키텍처 개편을 진행하는 개별 작업 브랜치입니다.

### 1. 이슈(Issue) 발행
코드 수정을 시작하기 전에, 먼저 프로젝트의 Github Issue 탭에 버그나 구현하려는 사양에 대해 작성하여 메인 관리자들과 방향성을 싱크해 주세요.

### 2. 풀 리퀘스트(Pull Request) 제출 규칙
* 변경 사항에 대한 작업은 반드시 `develop` 브랜치에서 파생된 `feature/` 브랜치에서 진행해 주세요.
* 풀 리퀘스트 대상 브랜치는 **`develop`** 브랜치여야 합니다.
